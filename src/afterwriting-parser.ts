...
import { Script, StructToken } from "./model/model";
...
let htmlreplacements: LexerReplacements = {
...
export function lexer(s: string, type: string, replacer: LexerReplacements, titlepage: boolean = false) {
  ...
  let styles = ['underline', 'italic', 'bold', 'bold_italic', 'italic_underline', 'bold_underline', 'bold_italic_underline'];
  let i = styles.length;
  let style;
  let match;
...
export function parse(original_script: string, cfg: any, generate_html: boolean) {
  let lastFountainEditor: vscode.Uri;
  let config = getFountainConfig(lastFountainEditor);
  let script = original_script;
  let result: Script = new Script(); // temporary, to be replaced by the real model
  let new_line_length = script.match(/\r\n/) ? 2 : 1;
  let lines = script.split(/\r\n|\r|\n/);
  let lines_length = lines.length;
  let current = 0;
  let scene_number = 1;
  let current_depth = 0;
  let match, text, last_title_page_token;
  let thistoken: token;
  let last_was_separator = false;
  let token_category = "none";
  let last_character_index;
  let dual_right;
  let state = "normal";
  let previousCharacter;
  let cache_state_for_comment;
  let nested_comments = 0;
  let title_page_started = false;
  ...
  function reduce_comment(prev: any, current: any) {
    if (current === "/*") {
      nested_comments++;
    } else if (current === "*/") {
      nested_comments--;
  ...
  function if_not_empty(a: any) {
    return a;
  }

  function updatePreviousSceneLength() {
    let action = result.lengthAction - lengthActionSoFar;
    let dialogue = result.lengthDialogue - lengthDialogueSoFar;
  ...
  function latestSectionOrScene(depth: number, condition: (token: StructToken) => boolean): StructToken {
    try {
      if (depth <= 0) {
        return null;
      } else if (depth == 1) {
        ...
        if (prevSection.children != null) {
          let lastChild = last(prevSection.children.filter(condition))
  ...
  function processInlineNote(text: string, linenumber: number): number {
    ...
    if (match = text.match(new RegExp(regex.note_inline))) {
      let level = latestSectionOrScene(current_depth + 1, () => true);
      if (level) {
        ...
        for (let i = 0; i < match.length; i++) {
          match[i] = match[i].slice(2, match[i].length - 2);
          result.properties.get().structure.push({ text: match[i], id: '/' + linenumber, isnote: true, children: [], level: 0, notes: [], range: new Range(new Position(linenumber, 0), new Position(linenumber, match[i].length + 4)), section: false, synopses: [] })
          irrelevantTextLength += match[i].length + 4;
        }
      }
    }
  ...
  function processDialogueBlock(token: token) {
  ...
  for (let i = 0; i < lines_length; i++) {
    ...
    if (text.trim().length === 0 && text !== "  ") {
      let skip_separator = (cfg.merge_multiple_empty_lines && last_was_separator) || (ignoredLastToken && result.tokens.length > 1 && result.tokens[result.tokens.length - 1].type == "separator");
    ...
    if (state === "title_page") {
      if (regex.title_page.test(thistoken.text)) {
        let index = thistoken.text.indexOf(":");
        ...
        if (keyformat) {
          ...
          result.titlePage[keyformat.position].push(thistoken);
        }
    ...
    function latestSection(depth: number): StructToken {
    ...
    if (state === "normal") {
      ...
      if (sceneHeadingMatch) {
        ...
        if (current_depth == 0) {
          ...
          let level = latestSection(current_depth);
          if (level) {
        ...
        if (result.properties.characters.has(character)) {
          let values = result.properties.characters.get(character);
  ...
  if (generate_html) {
    ...
    if (result.titlePage) {
      for (const section of Object.keys(result.titlePage)) {
        result.titlePage[section].sort(helpers.sort_index);
        titlehtml.push(`<div class="titlepagesection" data-position="${section}">`);
        let current_index = 0/*, previous_type = null*/;
        while (current_index < result.titlePage[section].length) {
          let current_token: token = result.titlePage[section][current_index];
    ...
    while (current_index < result.tokens.length) {
      let current_token: token = result.tokens[current_index];
      if (current_token.text != "") {
      ...
      if ((current_token.type == "action" || current_token.type == "centered") && !current_token.ignore) {
        ...
        if (current_index + 1 < result.tokens.length - 1) {
          ...
          let next_type = result.tokens[current_index + 1].type
        ...
        switch (current_token.type) {
          case 'scene_heading':
            let content = current_token.html;
